...
class Solver {
    ...
    public static int[] maxSums = { 0, 0, 0 };
    ...
    public void solve(String input) {
        ...
        Main.result = Arrays.stream(maxSums).reduce(0, Integer::sum);
        // Main.output += input;
    }

    public void adjustOrder(int tempSum) {
        ...
        if (tempSum >= maxSums[1]) {
            maxSums[2] = maxSums[1];
            maxSums[1] = tempSum;
            return;
        }

        if (tempSum >= maxSums[2]) {
            maxSums[2] = tempSum;
        }
    }
